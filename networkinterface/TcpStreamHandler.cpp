...
namespace SMNetwork
{
	...
	{
		bool bsplit = false;
		auto timeoutmillsecond = CONFINST.getTransportConfig()._timeout;
		_curpacklen = 0;
		if (retrycount > 100)
		{
			retrycount = 100;
		...
		while (!bsplit && tryindex++ < retrycount)
		{
			...
			SPDLOG_INFO("current recv data length {} from {}", len, getFullAddr());
			if (len > 0)
			{
			...
			case NetHeadType::FixPackLenPlaceHolder:
			{
				...
				{
					SPDLOG_WARN("only get message length {} need length {}", _recvbuf.size(), _curpacklen);
				}
			}break;
