...
template <ChannelType t, NetHeadType nethead, ChannelModeC modelc, EnumConcept MainType>
class TcpServerCombine
{
  public:
    ...
    asio::awaitable<void> _listen()
    {
        ...
        SPDLOG_INFO("channel {} in listen func, port {}", magic_enum::enum_name(_mainc),  addr);
        ...
        while(!_wantstop)
        {
            ...
            SPDLOG_INFO("listen port {} success get a new connect", _port);
            auto channel = LNM->createTcpChannel(make_unique<asio::ip::tcp::socket>(move(sock)), modelc, _mainc);
            assert(channel != nullptr);
            if (channel == nullptr)
            {
                SPDLOG_ERROR("create channel for mainc {} failed", magic_enum::enum_name(_mainc));
            }
            else
            {
                channel->start();
